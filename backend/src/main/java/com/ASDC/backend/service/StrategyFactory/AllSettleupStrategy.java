...
@Component
public class AllSettleupStrategy implements RetrievalStrategy<SettleUp> {

    private final SettleupPersistentService settleupPersistentService;

    public AllSettleupStrategy(SettleupPersistentService settleupPersistentService) {
        this.settleupPersistentService = settleupPersistentService;
    }

    @Override
    public List<SettleUp> retrieveItems(Room room) {
        return settleupPersistentService.getAllSettleUp(room.getId());
    }
}
